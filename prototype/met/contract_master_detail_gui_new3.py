# File: prototype/met/contract_master_detail_gui_new3.py - v5 updated for pricing_type + full field coverage
...
# adding new scheduling / unscheduling logic
...
class ScrolledTreeview(ttk.Frame):
    def __init__(self, parent, columns, height=5, headings=None, **kwargs):
        super().__init__(parent)
        ...
        for c in columns:
            heading = headings.get(c, c.replace("_", " ").title()) if headings else c.replace("_", " ").title()
            ...
            if c in ("id","active","quantity","priority","min_spots_per_day","max_spots_per_day","min_spots_per_week","max_spots_per_week","spot_length"): w=65
...
class ContractMasterDetailGUI:
    ...
    def build(self):
        ...
        rules_headings = {
            "min_spots_per_day": "Min/Day",
            "max_spots_per_day": "Max/Day",
            "min_spots_per_week": "Min/Wk",
            "max_spots_per_week": "Max/Wk",
            "allow_news": "News",
            "allow_special": "Special",
        }

        self.rules_tree_wrap = ScrolledTreeview(
            rules_frame,
            rules_cols,
            height=5,
            headings=rules_headings,
        )
        ...
        # Existing rule timing / options row
        ttk.Label(rf, text="Start:").grid(row=0, column=0, sticky="w")
        self.rule_start = tk.StringVar(value="06:00")
        ttk.Entry(rf, textvariable=self.rule_start, width=7).grid(row=0, column=1, padx=2)
        ...
        ttk.Checkbutton(rf, text="News", variable=self.allow_news).grid(row=0, column=4, padx=4)

        self.allow_spec = tk.IntVar(value=1)
        ttk.Checkbutton(rf, text="Special", variable=self.allow_spec).grid(row=0, column=5, padx=4)

        self.rule_active = tk.IntVar(value=1)
        ttk.Checkbutton(rf, text="Act", variable=self.rule_active).grid(row=0, column=6, padx=4)

        # Daily / weekly spot constraints
        rf_constraints = ttk.Frame(rules_frame)
        rf_constraints.pack(fill="x", padx=3, pady=1)

        ttk.Label(rf_constraints, text="Min/Day:").grid(row=0, column=0, sticky="w")
        self.min_per_day = tk.IntVar(value=0)
        ttk.Entry(
            rf_constraints,
            textvariable=self.min_per_day,
            width=4
        ).grid(row=0, column=1, padx=2)

        ttk.Label(rf_constraints, text="Max/Day:").grid(row=0, column=2, sticky="w")
        ttk.Label(rf_constraints, text="Max/Day:").grid(row=0, column=2, sticky="w")
        self.per_day = tk.StringVar(value="1")
        ttk.Entry(
            rf_constraints,
            textvariable=self.per_day,
            width=4
        ).grid(row=0, column=3, padx=2)

        ttk.Label(rf_constraints, text="Min/Wk:").grid(row=0, column=4, sticky="w")
        self.min_per_week = tk.IntVar(value=0)
        ttk.Entry(
            rf_constraints,
            textvariable=self.min_per_week,
            width=4
        ).grid(row=0, column=5, padx=2)

        ttk.Label(rf_constraints, text="Max/Wk:").grid(row=0, column=6, sticky="w")
        self.per_week = tk.StringVar(value="5")
        ttk.Entry(
            rf_constraints,
            textvariable=self.per_week,
            width=4
        ...
        self.schedule_run_btn = ttk.Button(
            sched_row,
            text="Run Schedule",
            command=self.run_schedule,
            state="disabled"
        )
        self.schedule_run_btn.pack(side="right", padx=5)

        self.new_schedule_run_btn = ttk.Button(
            sched_row,
            text="New Run Schedule",
            command=self.new_run_schedule,
            state="disabled"
        )
        self.new_schedule_run_btn.pack(side="right", padx=5)
    ...
    def update_button_states(self):
        ...
        if hasattr(self, 'new_schedule_run_btn'):
            self.new_schedule_run_btn.config(state=state_i)
    ...
    def load_rules_for_item(self, item_id):
        ...
        try:
           ...
           min_spots_per_day, max_spots_per_day,
           min_spots_per_week, max_spots_per_week,
           allow_news, allow_special_events, active, notes,
            ...
            for r in rows:
                ...
                self.rules_tree.insert(
                    ...
                    values=(
                        ...
                        r["min_spots_per_day"],
                        r["max_spots_per_day"],
                        r["min_spots_per_week"],
                        r["max_spots_per_week"],
                        r["allow_news"],
                        r["allow_special_events"],
                        r["active"],
                        pref_prog_disp,
                        pref_stop_disp,
                        r["notes"],
    ...
    def on_rule_select(self, event):
        ...
        self.min_per_day.set(
            r["min_spots_per_day"] if r["min_spots_per_day"] is not None else 0
        )

        self.per_day.set(
            "" if r["max_spots_per_day"] is None else str(r["max_spots_per_day"])
        )

        self.min_per_week.set(
            r["min_spots_per_week"] if r["min_spots_per_week"] is not None else 0
        )

        self.per_week.set(
            "" if r["max_spots_per_week"] is None else str(r["max_spots_per_week"])
        )
    ...
    def add_rule(self):
        ...
        try:
            max_per_day = (
                int(self.per_day.get().strip())
                if self.per_day.get().strip() != ""
                else None
            )

            max_per_week = (
                int(self.per_week.get().strip())
                if self.per_week.get().strip() != ""
                else None
            )
            cur.execute(
                """
                ...
                (
                    ...
                    self.days_var.get().strip(),
                    self.rule_start.get().strip(),
                    self.rule_end.get().strip(),
                    self.min_per_day.get() or 0,
                    max_per_day,
                    self.min_per_week.get() or 0,
                    max_per_week,
                    self.allow_news.get(),
                    self.allow_spec.get(),
                    self.rule_active.get(),
    ...
    def new_run_schedule(self):
        if not self.selected_item_id:
            messagebox.showwarning("Select Item", "Select Item first.")
            return

        try:
            result = analyze_contract_item(self.selected_item_id)
        except Exception as e:
        ...
        if result.get("status") != "ok":
            messagebox.showerror(
                "Analysis Error",
                result.get("message", "Unable to analyze Contract Item.")
            )
            return

        popup = tk.Toplevel(self.root)
        popup.title("New Run Schedule")
        ...
        report.append("NEW RUN SCHEDULE")
        report.append("=" * 60)
        report.append("")
        report.append(f"Contract Item: {result['contract_item_id']}")
        report.append(f"Quantity: {result['quantity']}")
        report.append(
            f"Flight: {result['start_date']} through {result['end_date']}"
        )
        report.append("")

        # Rule analysis
        report.append("RULE ANALYSIS")
        report.append("-" * 60)

        for rule_result in result["rules"]:
            rule = rule_result["rule"]
            math = rule_result["math"]

            report.append(
                f"Rule {rule_result['rule_number']}"
            )
            ...
            report.append(f"  Days: {days_text}")
            ...
            daily_text = (
                f"{min_day if min_day is not None else 0}"
                f" to "
                f"{max_day if max_day is not None else 'unlimited'}"
            )

            weekly_text = (
                f"{min_week if min_week is not None else 0}"
                f" to "
                f"{max_week if max_week is not None else 'unlimited'}"
            )

            report.append(f"  Daily: {daily_text}")
            report.append(f"  Weekly: {weekly_text}")
            report.append(
                f"  Eligible days: {math['total_eligible_days']}"
            )
            report.append(
                f"  Daily capacity: {math['daily_maximum']}"
            )
            report.append(
                f"  Weekly capacity: {math['weekly_maximum']}"
            )
            report.append(
                f"  Effective minimum: {math['effective_minimum']}"
            )
            report.append(
                f"  Effective maximum: {math['effective_maximum']}"
            )

            result_text = (
                "IMPOSSIBLE"
                if rule_result["impossible"]
                else "FEASIBLE"
            )

            report.append(f"  Result: {result_text}")

            for reason in math.get("impossible_reasons", []):
        ...
        report.append("CURRENT SCHEDULING STATUS")
        report.append("-" * 60)
        report.append(
            f"State: {current_status['scheduling_state']}"
        )
        report.append(
            f"Active spots: {current_status['active_count']}"
        )
        report.append(f"Pending: {counts['Pending']}")
        report.append(f"Scheduled: {counts['Scheduled']}")
        report.append(f"Exported: {counts['Exported']}")
        report.append(f"Completed: {counts['Completed']}")
        report.append(f"Cancelled: {counts['Cancelled']}")
        report.append("")
        ...
        if result["item_is_impossible"]:
            ...
            report.append("")
            report.append(
                "There is nothing more to schedule for this Contract Item."
            )
            ...
            report.append("")
            report.append(
                "The Contract Item can proceed to scheduling."
            )

            final_state = "feasible"
