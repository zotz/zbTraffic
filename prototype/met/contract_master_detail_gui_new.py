...
class ContractMasterDetailGUI:
    ...
    def update_button_states(self):
        ...
        if hasattr(self, 'add_item_btn'):
            ...
            self.item_status_lbl.config(text=f"→ Contract {self.selected_contract_id} selected" if has_contract else " (select a contract first)")
        
        if hasattr(self, 'add_rule_btn'):
            self.add_rule_btn.config(state=state_i)
            self.update_rule_btn.config(state="disabled")  # enabled on rule select
            self.rule_status_lbl.config(text=f"→ Item {self.selected_item_id} selected" if has_item else " (select an item first)")
