...
# File: prototype/met/contract_master_detail_gui.py - improved
# Keeps original 3-window layout, adds:
# 1) auto-select first contract item when contract selected -> Run Schedule enabled
# 2) preferred_program_id and preferred_stopset_id as dropdowns in bottom window with searchable combo for long lists
...
# ---------- Searchable Combobox for long lists (stopsets) ----------

class SearchableCombobox(ttk.Frame):
    """
    Better than plain combobox when list is long (preferred_stopset_id).
    Type to filter, shows label [id] but stores id.
    """
    def __init__(self, parent, options=None, placeholder="-- Any / No Preference --", width=30):
    ...
    def hide_if_not_focused(self):
        if self.listbox_frame:
            foc = self.focus_get()
            # keep open if focus is on listbox or buttons
    ...
    def on_keyrelease(self, e):
        ...
        if not text:
            ...
            self.filtered = [opt for opt in self.all_options if text in opt[1].lower() or text == str(opt[0]).lower()]
        self.show_list()
        self.refresh()
    ...
    def set_options(self, options, placeholder="-- Any / No Preference --"):
        self.all_options = [(None, placeholder)] + list(options)
        self.id_to_label = {oid: label for oid, label in self.all_options}
        self.filtered = self.all_options
        self.refresh()
        # reset selection if current id not in new list
        if self.var_id not in self.id_to_label:
            self.set_id(None)
...
class ContractMasterDetailGUI:
    def __init__(self, root):
        ...
        self.stations = {}
        self.commercials = {}
        self.programs = {}  # name -> id
        self.programs_by_id = {}
        self.stopsets_list = []  # list of (id, label)
        self.stopsets_by_id = {}
        self.contract_cache = {}
        self.item_cache = {}
        self.rule_cache = {}
    ...
    def build(self):
        ...
        # NEW: Preferred Program and Stopset row
        rf_pref = ttk.Frame(rules_frame)
        rf_pref.pack(fill="x", padx=3, pady=2)
        ttk.Label(rf_pref, text="Preferred Program:").grid(row=0, column=0, sticky="w")
        self.pref_program_cb = ttk.Combobox(rf_pref, state="readonly", width=30)
        self.pref_program_cb.grid(row=0, column=1, padx=5, sticky="w")

        ttk.Label(rf_pref, text="Preferred Stopset:").grid(row=0, column=2, sticky="w", padx=(15,2))
        self.pref_stopset_cb = SearchableCombobox(rf_pref, options=[], width=30)
        self.pref_stopset_cb.grid(row=0, column=3, padx=5, sticky="w")
        ttk.Label(rf_pref, text="(type to filter stopsets)", foreground="gray").grid(row=0, column=4, sticky="w")
    ...
    def update_button_states(self):
        ...
        self.item_status_lbl.config(
            text=f"→ Contract {self.selected_contract_id} selected" if has_contract else " (select a contract first)"
        )
        ...
        self.rule_status_lbl.config(
            text=f"→ Item {self.selected_item_id} selected" if has_item else " (select an item first)"
        )
    ...
    def load_lookups(self):
        ...
        try:
            ...
            # NEW: stopsets - long list
            try:
                # try different schema guesses
                for q in [
                    "SELECT id, COALESCE(name, code, 'Stopset ' || id) as label FROM stopsets ORDER BY label",
                    "SELECT id, name as label FROM stopsets ORDER BY label",
                    "SELECT id, code as label FROM stopsets ORDER BY label"
                ]:
                    try:
                        cur.execute(q)
                        rows = cur.fetchall()
                        if rows:
                            self.stopsets_list = [(r["id"], r["label"]) for r in rows]
                            self.stopsets_by_id = {r["id"]: r["label"] for r in rows}
                            break
                    except:
                        continue
                if self.stopsets_list:
                    self.pref_stopset_cb.set_options(self.stopsets_list)
            except Exception as e:
                print(f"stopsets lookup failed: {e}")
    ...
    def load_items_for_contract(self, contract_id):
        ...
        try:
            ...
            # ---- REQUIREMENT 1: auto-select first item ----
            if rows:
                ...
                # trigger on_item_select to enable Run Schedule
    ...
    def load_rules_for_item(self, item_id):
        ...
        try:
            ...
            for r in rows:
                prog_label = self.programs_by_id.get(r["preferred_program_id"], "") if "preferred_program_id" in r.keys() else ""
                stop_label = self.stopsets_by_id.get(r["preferred_stopset_id"], "") if "preferred_stopset_id" in r.keys() else ""
                # show id in column but label for readability
    ...
    def on_rule_select(self, event):
        ...
        # NEW: set preferred program/stopset
        prog_id = r["preferred_program_id"] if "preferred_program_id" in r.keys() else None
        stop_id = r["preferred_stopset_id"] if "preferred_stopset_id" in r.keys() else None
        ...
        if stop_id is not None:
            self.pref_stopset_cb.set_id(stop_id)
        else:
    ...
    def _get_pref_ids(self):
        ...
        if not prog_name or prog_name == "-- Any --":
            prog_id = None
        else:
            prog_id = self.programs.get(prog_name)
        ...
        try:
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
             """, (self.selected_item_id, self.days_var.get().strip(), self.rule_start.get().strip(), self.rule_end.get().strip(),
                  ...
                  self.notes_var.get().strip(), prog_id, stop_id, now_str(), now_str()))
            con.commit()
        ...
        try:
            ...
            WHERE id=?
             """, (self.days_var.get().strip(), self.rule_start.get().strip(), self.rule_end.get().strip(),
                  self.per_day.get() or 0, self.per_week.get() or 0, self.allow_news.get(), self.allow_spec.get(), self.rule_active.get(),
                  self.notes_var.get().strip(), prog_id, stop_id, now_str(), rid))
            con.commit()
            self.load_rules_for_item(self.selected_item_id)
        finally:
            con.close()
